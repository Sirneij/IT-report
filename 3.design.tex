...
\subsection{Flask applications deployment}
\begin{itemize}
	...
	\mint{bash}{~$ ssh jidogun@10.50.0.9}
	...
	\item The \mintinline{nginx}{nginx} proxy server was started.
	\mint{bash}{~$ sudo /etc/init.d/nginx start}
	\item As expected, \mintinline{nginx}{nginx} comes with a default configuration file which can be found in a \mintinline{nginx}{/etc/nginx/sites-enabled/default}. This must be removed to allow personalised configuration file.
	...
	where \mintinline{bash}{file_name} is the desired name given to the file. It was called \mintinline{bash}{flask_settings} in my case, so:
	\mint{bash}{~$ sudo touch /etc/nginx/sites-available/flask_settings}
	\item Since the just created file needs to be active for its contents to be used by \mintinline{nginx}{nginx}, a symbolic file, pointing to the \mintinline{nginx}{flask_settings} file created above, was made in \mintinline{nginx}{/etc/nginx/sites-enabled/flask_settings}.
	\mint{bash}{~$ sudo ln -s /etc/nginx/sites-available/flask_settings /etc/nginx/sites-enabled/flask_settings}
	\item The symbolic file made above was edited using \mintinline{nginx}{nano} editor, filling it with the short Snippet below:
	\mint{bash}{~$ sudo nano /etc/nginx/sites-enabled/flask_settings}
	\begin{code}
		\captionof{listing}{/etc/nginx/sites-enabled/flask\_settings}
		\begin{minted}
[
frame=lines,
framesep=2mm,
...
server{
	...
	\mint{bash}{~$ sudo /etc/init.d/nginx restart}
	\item With the above configurations, everything regarding \mintinline{nginx}{nginx} was pretty much done with. The next inline was to set up Python and the application as highlighted below. 
	\begin{itemize}
		\item A virtual environment for the application to be deployed must be created to avoid Python version conflict issues. But First, \mintinline{nginx}{virtualenv} was installed.
		\mint{bash}{~$ sudo pip install virtualenv}
		\item For proper organization, a directory for applications built using Flask was made.
		\mint{bash}{~$ mkdir flask_app}
		\item Next, directory was changed into the newly created one.
		\mint{bash}{~$ cd flask_app}
		\item The application to be deployed was cloned from its git repository which was \url{https://github.com/Sirneij/Blog.git}.
		\mint{bash}{~/flask_app$ git clone https://github.com/Sirneij/Blog.git}
		\item Directory was changed into the new application folder.
		\mint{bash}{~/flask_app$ cd Blog}
		\item Next, a virtual environment was created for it.
		\mint{bash}{~/flask_app/Blog$ virtualenv venv #Environment name.}
		\item Having created the virtual environment, it was activated.
		\mint{bash}{~/flask_app/Blog$ source venv/bin/activate}
		...
		\mint{bash}{(venv):~/flask_app/Blog$ sudo pip install -r requirements.txt gunicorn}
		...
		\mint{bash}{(venv):~/flask_app/Blog$ gunicorn run:app}
		That was it! Application was successfully deployed!
	\end{itemize}
	\item To confirm that the app was successfully deployed, a browser was visited and the ip address of the server provided was inputted as \ac{URL} or web address which returned a full version of the initial app.
\end{itemize}